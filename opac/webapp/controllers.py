...
def get_issues_by_jid(jid, **kwargs):
    ...
    if order_by:
        del kwargs['order_by']
    order_by = order_by or ["-year", "-volume", "-order"]
    issues = Issue.objects(journal=jid, **kwargs)
    if issues:
        return issues.order_by(*order_by)
...
def get_articles_by_iid(iid, **kwargs):
    ...
    if len({a.publication_date for a in articles}) > 1:
